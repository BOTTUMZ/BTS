...
settings = {
    ...
    "man2":"ยินดีต้อนรับ",
    "man3":"อ่านกฏด้วย",
    "message":"บัญชีนี้ได้การป้องกันโดย 🔥Hack Scan 🔔win🔔🇹🇭  ระบบได้ทำการบล็อคคุณอัตโนมัติ\nติดต่อเชลบอทรึบอทวี10คอมเม้นในโพสเลยคับ",
...
#==============================================================================#
                    ...
                    if mic == "on":
                        ...
                        line.createGroup("MIN HACK Group",[key])
                        line.sendText(msg,to,"MIN HACK")
                elif "ลบค้างเชิญ " in msg.text.lower():
                    spl = re.split("ลบค้างเชิญ ",msg.text,flags=re.IGNORECASE)
                    if spl[0] == "":
                        spl[1] = spl[1].strip()
...
#==============================================================================#
                    ...
                    if msg.toType == 2:
                        ...
                        if group.preventedJoinByTicket == False:
                            ...
                            line.sendMessage(to, "Link Qr to open")
                elif "โชคดีคับ" in msg.text:
                        if msg.toType == 2:
                         _name = msg.text.replace("โชคดีคับ","")
                         gs = line.getGroup(receiver)
                         line.sendMessage(receiver,"ขอให้ทุกคน ร่ำรวย ตลอดไป ô")
                         targets = []
                         for g in gs.members:
                             if _name in g.displayName:
                                 targets.append(g.mid)
                         if targets == []:
                             line.sendMessage(receiver,"สาธุ....")
                         else:
                             for target in targets:
...
#==============================================================================#
                    ...
                    bc = msg.text.replace("#fbc: ","")
                    gid = line.getAllContactIds()
                    for i in gid:
                        line.sendText(i,"======[ข้อความประกาศแชท]======\n\n"+bc+"\n\nBy: MIN HACK SELFBOT!")
        
                elif "#pgbc: " in msg.text:
                    bc = msg.text.replace("#pgbc: ","")
                    gid = line.getGroupIdsJoined()
                    for i in gid:
                        line.sendImageWithURL(i, bc)
                    
                elif "#pfbc: " in msg.text:
                    bc = msg.text.replace("#pfbc: ","")
                    gid = line.getAllContactIds()
                    for i in gid:
                        line.sendImageWithURL(i, bc)
